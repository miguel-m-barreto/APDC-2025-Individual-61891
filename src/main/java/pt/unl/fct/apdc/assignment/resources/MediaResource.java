...
//import google cloud service util
//import pt.unl.fct.apdc.assignment.util.GcsUtil;


@Path("/media")
public class MediaResource {

	//private static final Bucket BUCKET = StorageOptions.getDefaultInstance().getService().get("shining-expanse-453014-c4.appspot.com");
	...
	@SuppressWarnings("deprecation")
	@POST
	@Path("/upload/public/{object}")
	public Response uploadPublicFile(@PathParam("object") String object,
			@HeaderParam("Content-Type") String contentType,
			@Context HttpServletRequest request) {
		...
		// Upload to Google Cloud Storage (see Google's documentation)
		Storage storage = StorageOptions.getDefaultInstance().getService();
		object = "userphoto-" + object + java.util.UUID.randomUUID(); // auto filename
		BlobId blobId = BlobId.of(bucket, object);
